  ...
  render: function() {
      ...
      if(cell.isOpened()){
          ...
          if(mineCount && !cell.isFlagged()){
              mineCountElement = <span className={"mine-count-"+ mineCount}>{mineCount}</span>
          }
      }
      if(iconClassName){
          icon = <i className={iconClassName}></i>
      }
      return <div className={cellClassName} onClick={this._onClick} onContextMenu={this._onRightClick}>
              {icon}
              {mineCountElement}
          </div>;
  },
          ...
          for(var i =0; i < rowsCount; i++){
              ...
              for(var j =0; j < columnsCount; j++){
                  rows.push(<CellRC className="cell" 
                          cell={this.props.board.getCell(i, j)} 
                          onClick={this.props.onBoardCellClick} 
                          onRightClick={this.props.onBoardCellRightClick}
                          row={i}
                          column={j}
                          key={j} />)
              }
              cells.push(<div className="rows" key={i}>{rows}</div>) 
          }
      }
...
var HeaderRC = React.createClass({
  ...
  render: function() {
      ...
      return <div className="header">
                  ...
                  <div className={"status-message " + this.props.status}>{statusMessage}</div>
...
export default React.createClass({
  ...
  render: function() {
      ...
      return <div className="mine-sweeper">
              <HeaderRC
                  remainingFlags={this.state.board.getRemainingFlags()}
                  onButtonClick={this._createNewBoard}
                  status={this.state.board.getStatus()}
              />
              <BoardRC 
                  board={this.state.board}
                  onBoardCellClick={this._onBoardCellClick}
                  onBoardCellRightClick={this._onBoardCellRightClick}
              />
          </div>;
  },
